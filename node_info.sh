...
while test $# -gt 0; do
	...
	-h|--help)
		...
		echo
		echo -e "${C_LGn}Functionality${RES}: the script shows information about a Calamari node"
		echo
		...
		echo -e "https://github.com/SecorD0/Calamari/blob/main/node_info.sh - script URL"
...
main() {
	...
	# Send Pull request with new texts to add a language - https://github.com/SecorD0/Calamari/blob/main/node_info.sh
	#elif [ "$language" = ".." ]; then
	else
		local t_nn="\nMoniker:                  ${C_LGn}%s${RES}"
